...
NUEVO: además de guardar máscaras, puede guardar imágenes "MASKED"
aplicando la máscara sobre la imagen original.

Salida:
- Máscaras:  OUTPUT_DIR/<subcarpeta>/<imagen>_mask.tiff
- Masked:   MASKED_DIR/<subcarpeta>/<imagen>.tiff   (mismo nombre que la imagen)

Notas:
- Si INVERT_MASK=True, final_mask = invert(mask). OJO: para masked conviene usar
  la máscara POSITIVA (zona a eliminar) antes de invertir.
"""
...
# NUEVO: carpeta para imágenes enmascaradas
MASKED_DIR   = f"TomasMasked/Cam{Camara}"
...
# --- Manejo de subcarpetas existentes en OUTPUT_DIR ---
SKIP_IF_EXISTS    = True

# NUEVO: guardar imagen masked (aplica máscara a la imagen)
SAVE_MASKED       = True
MASKED_VALUE      = 255         # valor a poner donde la máscara es 1 (0 = negro)
MASKED_FORMAT_EXT = ".tiff"    # ext de salida (".tiff" recomendado)
# =========================
...
def apply_mask_to_image(original: Image.Image, final_mask: Image.Image, masked_value: int = 0) -> Image.Image:
    """
    Aplica máscara con la convención:
    255 = zona válida (se conserva)
    0   = zona a bloquear (se pone masked_value)
    """
    M = np.array(final_mask, dtype=np.uint8) == 0  # True donde BLOQUEAR
...
def process_image(model: YOLO, img_path: Path, out_mask_dir: Path, out_masked_dir: Optional[Path]) -> None:
    with Image.open(img_path) as im:
        im.load()
        # Guardamos original y gray para overlay/modelo
        original = im.copy()
        gray = ensure_gray(im)
    ...
    # máscara positiva: 255 donde detecta (zona a enmascarar)
    positive_mask = Image.new("L", gray.size, color=0)
    if results:
        r = results[0]
        drew_masks = draw_segmentation_masks(positive_mask, r)
        if not drew_masks:
            draw_boxes(positive_mask, r)
        positive_mask = postprocess_mask(positive_mask)

    # máscara final para guardar (según INVERT_MASK)
    final_mask = ImageOps.invert(positive_mask) if INVERT_MASK else positive_mask

    # --- Guardar máscara ---
    out_mask_dir.mkdir(parents=True, exist_ok=True)
    out_mask_path = out_mask_dir / f"{img_path.stem}_mask.tiff"
    final_mask.convert("L").save(out_mask_path, format="TIFF", compression="raw")

    # --- Guardar overlay (opcional) ---
    if OVERLAY_ORIGINAL:
        overlay_img = create_overlay(gray, positive_mask, OVERLAY_ALPHA)
        out_overlay = out_mask_dir / f"{img_path.stem}_overlay.tiff"
        overlay_img.save(out_overlay, format="TIFF", compression="raw")

    # --- Guardar imagen masked (NUEVO) ---
    if SAVE_MASKED and out_masked_dir is not None:
        out_masked_dir.mkdir(parents=True, exist_ok=True)
        masked_img = apply_mask_to_image(original, positive_mask, masked_value=MASKED_VALUE)

        # Mantener nombre original y forzar extensión deseada
        out_masked_path = out_masked_dir / f"{img_path.stem}{MASKED_FORMAT_EXT}"
        # Guardar TIFF sin compresión (robusto para PIV)
        masked_img.save(out_masked_path, format="TIFF", compression="raw")
...
def main():
    ...
    if PROCESS_RECURSIVE:
        ...
        for sf in subfolders:
            out_sf_mask = output_dir / sf.name
            out_sf_masked = (masked_dir / sf.name) if SAVE_MASKED else None

            # Manejo de carpetas existentes
            if not prepare_output_subfolder(out_sf_mask):
                continue
            if SAVE_MASKED:
                # Si borramos máscaras, conviene borrar masked también (para consistencia)
                if out_sf_masked.exists() and DELETE_EXISTING:
                    shutil.rmtree(out_sf_masked)
            ...
            for f in tqdm(files, desc=f"Procesando {sf.name}", leave=False):
                try:
                    process_image(model, f, out_sf_mask, out_sf_masked)
                except Exception as e:
                    print(f"[ADVERTENCIA] Error en {sf.name}/{f.name}: {e}")

        print(f"Listo. Máscaras guardadas en: {output_dir}")
        if SAVE_MASKED:
            print(f"Listo. Imágenes MASKED guardadas en: {masked_dir}")
        ...
        for f in tqdm(files, desc="Procesando"):
            try:
                process_image(model, f, output_dir, masked_dir if SAVE_MASKED else None)
            except Exception as e:
